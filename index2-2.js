...
const monsterImgs = ['Enermy/E1.png', 'Enermy/E2.png','Enermy/s1.png','Enermy/s1.png','Enermy/s1.png']
...
function createMonster() {
  ...
  newMonster.style.top = `${Math.floor(Math.random() * 400)}px`// easy mode
...
function moveMonster(monster) {
  let moveMonsterInterval = setInterval(() => {
    ...
    if (xPosition <= 0) {
      ...
      monster.style.left = `${xPosition - 10}px`
    }
  }, 30)
}
...
function playGame() {
  ...
  justice = new Audio("Sound/boss.mp3")
  justice.play()
